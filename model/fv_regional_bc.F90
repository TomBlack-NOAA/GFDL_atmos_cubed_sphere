...
!
      integer,save :: cld_amt_index                                    &   !<--
                     ...
                     ,liq_water_index                                  &   !<--  tracer variables
                     ,o3mr_index                                       &   !<--  in the tracer
...
!  write(0,*)' regional_bc_data before read_regional_bc_file'
!  write(0,*)' nt=',nt,' var_name_root=',trim(var_name_root),' index=',index
...
!
      type(fv_atmos_type),target,intent(inout) :: Atm                      !<-- Atm object for the current domain
!
      integer :: dimid,index,istat,n,na,natts                           &
                ,ncid_core,ncid_core_new                                &
                ,ncid_tracers,ncid_tracers_new                          &
                ,nctype,ndims,ngatts,nkount,nn,nv_core,nv_tracers       &
                ,var_id
!
...
!
!-----------------------------------------------------------------------
!***  Now take care of the tracer restart file.
!-----------------------------------------------------------------------
!
...
!
      ...
      jstart_g=jsg-halo
      jend_g  =jeg+halo
!  write(0,*)' write_full istart_g=',istart_g,' iend_g=',iend_g,' jstart_g=',jstart_g,' jend_g=',jend_g
!
...
!-----------------------------------------------------------------------
!
      lbnd_x_tracers=lbound(Atm(1)%q,1)
      ubnd_x_tracers=ubound(Atm(1)%q,1)
      lbnd_y_tracers=lbound(Atm(1)%q,2)
      ubnd_y_tracers=ubound(Atm(1)%q,2)
!
...
!
        do k=1,nz
!         global_field(istart_g:iend_g, jstart_g:jend_g, k) = 0.0          !<-- Set global array layer to zero.
          call mpp_gather(istart,iend,jstart,jend                                    &
